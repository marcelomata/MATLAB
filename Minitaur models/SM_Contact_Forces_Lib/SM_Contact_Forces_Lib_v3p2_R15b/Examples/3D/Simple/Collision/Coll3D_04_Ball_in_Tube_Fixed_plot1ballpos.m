...
tube_h = surf(temp_tubez*tube.length-tube.length/2,temp_tubex*tube.inner_rad,temp_tubey*tube.inner_rad);
...
ball_h = surf(...
    temp_ballx*ball.rad-temp_bpz(1),...
    temp_bally*ball.rad-temp_bpx(1),...
    temp_ballz*ball.rad+temp_bpy(1));
...
hold off
axis equal
axis([[-1 1]*tube.length/2*2 [-1 1 -1 1]*tube.outer_rad])
