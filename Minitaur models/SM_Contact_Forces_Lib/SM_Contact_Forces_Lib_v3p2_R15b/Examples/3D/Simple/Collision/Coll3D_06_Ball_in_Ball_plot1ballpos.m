...
outerball_h = surf(...
    temp_ballx*outer_ball.inner_rad,...
    temp_bally*outer_ball.inner_rad,...
    temp_ballz*outer_ball.inner_rad);
set(outerball_h,'EdgeColor','none','AmbientStrength',1,'FaceColor',[0.0 0.5 0.7],'FaceAlpha',0.4);

ball_h = surf(...
    temp_ballx*inner_ball.rad+temp_bpx(1)*ones(size(temp_ballx)),...
    temp_bally*inner_ball.rad+temp_bpy(1)*ones(size(temp_bally)),...
    temp_ballz*inner_ball.rad+temp_bpz(1)*ones(size(temp_ballz)));
