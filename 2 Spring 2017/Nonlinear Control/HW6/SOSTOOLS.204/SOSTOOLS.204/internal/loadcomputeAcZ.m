function maplecmd = loadcomputeAcZ
% LOADCOMPUTEACZ --- This is called by getequation.m
%
...
maplecmd = [...
'FPexpr := convert(expr,float):',...
'FPexpr := simplify(FPexpr):',...
'FPexpr := expand(FPexpr):',...
'   ',...
'F := [coeffs(FPexpr,vartable,''Z'')]:',...
'ndecvars := nops(decvartable);',...
'   ',...
'nequs := nops(F):',...
'val := eval(F,[seq(decvartable[k]=0,k=1..ndecvars)]):',...
'F := F - val:',...     % Eliminating constant terms.
];
