...
hessfunc = ahess(@(joints)fk(joints,linkLengths,target), 3*numLinks);
%do optimization
options = optimoptions(@fmincon,'Display','iter','MaxFunEvals',2500,'Algorithm','interior-point','SpecifyObjectiveGradient',true,'CheckGradients',true, 'HessianFcn', hessfunc);
[answer,fval,exitflag]=fmincon(@(q0)criterionGradient(q0,target,linkLengths,min_roll,min_pitch,min_yaw,max_roll,max_pitch,max_yaw),q0,[],[],[],[],lb,ub,@(q0)constraints(q0,linkLengths,target),options);
